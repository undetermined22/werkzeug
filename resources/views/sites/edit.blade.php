                    ...
                    <form action="{{ route('sites.update', $site) }}" method="POST">
                        <div class="overflow-hidden sm:rounded-md">
                            <div class="px-4 py-5 bg-white sm:p-6">
                                <div class="mb-10">
                                    <h3 class="text-lg font-medium leading-6 text-gray-900">Edit site</h3>
                                    @csrf
                                    @method('PUT')
                                </div>
                                @if ($errors->any())
                                ...
                                <div class="grid grid-cols-6 gap-6">
                                    <div class="col-span-6">
                                        ...
                                        <input type="text" name="name" id="name" value="{{ old('name') ?? $site->name }}" required class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                                    </div>
                                    ...
                                    <div class="col-span-6 sm:col-span-3">
                                        ...
                                        <select id="server-id" name="server_id" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                            ...
                                            @foreach ($servers as $server)
                                                <option value="{{ $server->id }}" @selected((old('server_id') ?? $site->server_id) == $server->id)>{{ $server->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div class="col-span-6 sm:col-span-3">
                                        ...
                                        <select id="domain-id" name="domain_id" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                            ...
                                            @foreach ($domains as $domain)
                                                <option value="{{ $domain->id }}" @selected((old('domain_id') ?? $site->domain_id) == $domain->id)>{{ $domain->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div class="col-span-6">
                                        ...
                                        <textarea name="description" id="description" value="{{ old('description') ?? $site->description }}" rows="4" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"></textarea>
                                    </div>
